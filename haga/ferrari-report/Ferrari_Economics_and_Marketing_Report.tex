% !TeX program = xelatex
% XeLaTeX academic report (Ferrari case study)
...
Ahmed Raafat Salah (Sec. 1, BN 3)\\
Ahmed Mohamed Mahmoud (Sec. 1, BN 6)\\
Rania Hany Heider (Sec. 1, BN 22)\\
Omar Mostafa Mahmoud (Sec. 2, BN 1)\\
Ahmed Hesham Mohammed (Sec. 1, BN 9)\\
Reem Osama Ahmed (Sec. 1, BN 24)\\
Abdelrahman Ahmed Kotb Abdelaziz (Sec. 1, BN 32)\\
Mohammed Abd Allah (Sec. 2, BN 20)\\
Bassant Mahmoud (Sec. 1, BN 13)\\
Mohammed Gamal Kassem (Sec. 2, BN 14)%
}
\newcommand{\ReportAuthorPdf}{Ahmed Raafat Salah; Ahmed Mohamed Mahmoud; Rania Hany Heider; Omar Mostafa Mahmoud; Ahmed Hesham Mohammed; Reem Osama Ahmed; Abdelrahman Ahmed Kotb Abdelaziz; Mohammed Abd Allah; Bassant Mahmoud; Mohammed Gamal Kassem}
\newcommand{\ReportAffiliation}{Faculty of Engineering, Aerospace Department, Cairo University}
\newcommand{\ReportCourse}{Economics and Marketing}
\newcommand{\ReportInstructor}{Dr. Hossam Shahein}
\newcommand{\ReportDate}{December 2025}

% PDF metadata (Title/Author fields in the exported PDF)
\hypersetup{
  pdftitle={\ReportTitle},
  pdfauthor={\ReportAuthorPdf},
  pdfsubject={\ReportCourse},
  pdfkeywords={Ferrari, economics, marketing, luxury, pricing, brand equity}
}
...
\begin{tabular}{@{}p{0.22\textwidth}p{0.72\textwidth}@{}}
        \textbf{Authors} & \parbox[t]{0.72\textwidth}{\ReportAuthor}\\
...
This report does not rely on proprietary internal data. Quantitative claims (e.g., exact financials, production volumes, conversion rates) are avoided unless sourced from official disclosures. Where numerical analysis would typically be expected, the report provides structured tables/charts for later insertion and explains the logic and the type of data required.
...
\caption{Business model diagram.}
\end{figure}
...
@{}}
\caption{Illustrative decision-rights matrix for a luxury automotive firm.}\label{tab:decision_rights}\\
  	\toprule
...
\section{Website Evaluation Framework}
This section proposes a structured evaluation framework. Students can apply it empirically by observing pages, features, and flows.
...
% Chapter 7
% ============================================================
\chapter{The Economics of Artificial Scarcity}
...
\begin{table}[htbp]
    \centering
    \caption{STP summary matrix}
    \label{tab:stp_matrix}
    \begin{tabularx}{\linewidth}{@{} l X @{}}
        \toprule
      	\textbf{STP element} & \textbf{Ferrari case notes (Evidence-based analysis)} \\
        \midrule
      	\textbf{Segmentation} & \textbf{Demographic:} High-income professionals, entrepreneurs, and ultra-high-net-worth individuals (UHNWIs). \newline
      	\textbf{Psychographic \& Behavioral:}
        \begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
            \item \textit{Collectors:} Motivated by rarity, heritage, and portfolio value.
            \item \textit{Drivers/Enthusiasts:} Motivated by performance and driving experience.
            \item \textit{Status Seekers:} Motivated by visible prestige and social signaling.
            \item \textit{Brand Community:} Motivated by belonging, events, and relationships.
  \end{itemize} \\
        \addlinespace
          	\textbf{Targeting} & \textbf{Priority:} Prioritizes clients who align with "brand stewardship," specifically long-term loyal owners, engaged community participants, and those valuing authenticity. \newline
          	\textbf{Logic:} Strategy aims to maximize lifetime value while minimizing the risk of brand dilution. \\
        \addlinespace
          	\textbf{Positioning} & \textbf{Core Statement:} An iconic luxury performance brand with authentic racing heritage. \newline
          	\textbf{Differentiators:} Offers exceptional engineering and craftsmanship through scarce, highly desirable products and experiences. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{PESTEL analysis for Ferrari (qualitative).}
...
\begin{table}[H]
\centering
\caption{Brand equity diagnostic.}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Provide transparent reporting and measurable initiatives (see Appendix for details).
...
\toprule
  	\textbf{Initiative} & \textbf{Time horizon} & \textbf{Key actions and success metrics}\\
\midrule
...
% ============================================================
\appendix
\chapter{Appendix A: Expanded Marketing Mix (7Ps) for Ferrari}
\section{Product}
...
\section{Price}
Discuss premium pricing, line pricing, psychological pricing in luxury, and allocation mechanisms. Optionally add an elasticity analysis subsection if needed.
...
\chapter{Appendix D: Competitive Set and Positioning Map}
...
The TOWS matrix translates SWOT observations into actionable strategies.
...
\begin{longtable}{@{}
  ...
  >{\raggedright\arraybackslash}p{0.41\textwidth}
@{}}
\caption{TOWS matrix (SWOT translated into strategies).}\label{tab:tows}\\
  	\toprule
...
\endlastfoot
        ...
        \textbf{Strengths (S)} & \textbf{SO Strategies} & \textbf{ST Strategies}\\
 & Ferrari can leverage its brand reputation and racing-derived capabilities to capitalise on hybrid and electric luxury mobility. Transfer of Formula 1 innovations to road cars helps meet stringent sustainability expectations while enhancing performance. Additionally, Ferrari can target emerging luxury markets with exclusive, limited-edition models and bespoke personalisation to grow revenue without compromising engineered scarcity.
 & Ferrari’s loyalty and pricing power buffer external threats such as environmental regulation and intensified competition. By maintaining leadership in powertrain efficiency and alternative-energy research, the firm stays proactive in the face of mandates. Simultaneously, artisanal craftsmanship and heritage protect the brand from price wars and reduce exposure to macroeconomic downturns.\\

\addlinespace[0.8em]
        \textbf{Weaknesses (W)} & \textbf{WO Strategies} & \textbf{WT Strategies}\\
 & High production costs and limited scalability can be mitigated by integrating digital ecosystems and customer data analytics. Better digital engagement supports demand forecasting and relationship management, reducing operational overhead. Ferrari can also address R\&D gaps in electrification via selective partnerships with specialist technology firms while protecting its design DNA.
 & To defend against combined internal vulnerabilities and external threats, Ferrari should maintain strict control over licensing and brand extensions to prevent dilution and ensure collaborations reinforce premium standing. Financial discipline and flexible production planning help navigate volatility; keeping supply intentionally below demand preserves brand equity and limits financial risk.\\
\end{longtable}
...
% This appendix provides a structured checklist of figures and tables in this report.
...
% Figure 4 & Ferrari website UX evidence\\
